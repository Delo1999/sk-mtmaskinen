console.log('Welcome to the joke factory! Let me tell you something about programming:')

function getRandomInt(max) {
	return Math.floor(Math.random() * max);
}

function slumpaSkämt(jokes, antal) {
	let randomJokes = [];
	for (let i = 0; i < antal; i++) {
		let randomIndex = getRandomInt(jokes.length);
		randomJokes.push(jokes[randomIndex]); //vad händer här?
	}
	return randomJokes;
...
let skämt = slumpaSkämt(programmingJokes, 2);

skämt.forEach(skämt => console.log(skämt));
